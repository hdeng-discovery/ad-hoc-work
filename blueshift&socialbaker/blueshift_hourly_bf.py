...
if len(str(date.today().day))==1 and len(str(date.today().month))==1:
    prefix_1='discoveryplus.com/events/{}/{}/{}/'.format(date.today().year,'0'+str(date.today().month),'0'+str(date.today().day))
    prefix_2='discoveryplus.com/events/{}/{}/{}/'.format(date.today().year,'0'+str(date.today().month),'0'+str(date.today().day-1))
elif len(str(date.today().day))==1:
...
elif len(str(date.today().month))==1:
    prefix_1='discoveryplus.com/events/{}/{}/{}/'.format(date.today().year,'0'+str(date.today().month),date.today().day)
    prefix_2='discoveryplus.com/events/{}/{}/{}/'.format(date.today().year,'0'+str(date.today().month),date.today().day-1)
else:
...
if datetime.today().hour!=0:
    for result in paginator.paginate(Bucket = bucket, Prefix=prefix_1):
        for file in result.get('Contents', []):
            ...
            # if (int(file_name.split('.')[-3].split('-')[-2][-6:]) >= (datetime.today().hour-1)*10000) & \
            #     (int(file_name.split('.')[-3].split('-')[-2][-6:]) <= (datetime.today().hour+1)*10000):
